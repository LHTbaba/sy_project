		 ...
		 <view class="rlist">
		    <view class="rlist_item" v-for="(item,index) in list" @click="toRhhs(item.type)" :key="index">
				...
				<view class="rhs_top">
					...
					<view class="rhs_top_middle"  >
						<view class="name_strong"  v-if="item.type=='1'" style="color: rgb(37, 183, 110)">
							{{item.name}}
						</view>
						<view class="name_strong"  v-if="item.type=='2'" style="color: rgb(242, 168, 59)">
							{{item.name}}
						</view>
						<view class="name_strong"  v-if="item.type=='3'" style="color: rgb(59, 143, 242)">
							{{item.name}}
						</view>
						<text style="color: #8f8f94;">数据来源</text>
					</view>
					<view class="rhs_top_right">
						<view class="team_nums">
							<text class="name_strong" style="color: rgb(38, 51, 67);margin-right: 10rpx;">{{item.nums}}</text>人
						</view>
						<text style="color: #8f8f94;">家庭人口数</text>
					</view>
		 	  	</view>
				<view class="rhs_bottom">
					<text class="local_icon"></text>{{item.address}}<text class="xz_icon">{{item.status}}</text> 
				</view> 
		    </view>
		 </view>	 
