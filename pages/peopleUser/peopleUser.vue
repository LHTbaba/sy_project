		...
		<view class="wrap" id='wrap' style="position: relative;background: #F8F8F8;">
			...
			<scroll-view :style="{height:scrollerHeight}" scroll-y="true">
			    <view class="user_box">
			    	<view class="user_left">
						...
						<view class="user_name">
							...
							<text class="hui_name" style="color: rgba(255, 255, 255,.8);font-size: 28rpx;">申报人姓名</text>
						</view>
			    	</view>
					<text class="user_right" @click="changeZh()">
						切换账号
					</text>
			    </view>
				<!-- <view class="msg_box">
					<view class="msg_list">
						...
						<view class="msg_item">
							...
							<text class="msg_hui">享受措施</text>
						</view>
						<view class="msg_item">
							<view class="msg_num" style="color: rgb(231, 98, 75);">
								2
							</view>
							<text class="msg_hui">享受金额</text>
						</view>
					</view>
				</view> -->
				<view class="user_list">
					...
					<view class="user_card" @click="loginOut1()">
...
<script>
	export default {
		...
		mounted() { 
			...
			this.poorname=uni.getStorageSync('poorName');
		...
		methods: {
					...
					success: function(res) {
						...
						if (res.subscriptionsSetting.mainSwitch) { // 用户打开了订阅消息总开关
							if (res.subscriptionsSetting.itemSettings != null) { // 用户同意总是保持是否推送消息的选择, 这里表示以后不会再拉起推送消息的授权
								let moIdState = res.subscriptionsSetting.itemSettings['ThgIlxwPDhqXL1hbqbH9xzSm92Re4ZBGn0e1O0-FPmA']; // 用户同意的消息模板id
								if (moIdState === 'accept') {
									console.log('接受了消息推送');
									wx.requestSubscribeMessage({ // 调起消息订阅界面
										tmplIds: ['ThgIlxwPDhqXL1hbqbH9xzSm92Re4ZBGn0e1O0-FPmA'],
										success(res) {
											console.log('订阅消息 成功 ');
											console.log(res);
											_this.dyNums();
			...
			loginOut1() {
				...
				var userCard = uni.getStorageSync('userCard');
				var _this = this;
				uni.showModal({
					content: '确定要解绑账号吗？',
					success: function(res) {
						if (res.confirm) {
							_this.$api.jbpOpenid({
								'cmd.OPENID': openId,
								"cmd.USERCARD": userCard
							}).then(res => {
								console.log(333);
								console.log(res);
								if (res.data.TIPS == '1') {
									_this.$toast(res.data.SUCESS);
									uni.removeStorageSync('sessionId');
									uni.removeStorageSync('nickName');
									uni.removeStorageSync('avatarUrl');
									uni.removeStorageSync('openId');
									uni.removeStorageSync('userCard');
									uni.removeStorageSync('poorName');
									setTimeout(function() {
										uni.reLaunch({
											url: '/pages/people/people',
										});
